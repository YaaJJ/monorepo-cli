...
setTimeout(() => {
  ...
  // const gitCloneDir = path.resolve(__dirname, '..', 'my-test1');
  console.log(gitCloneDir, 5555555555555555555);
  try {
    // rimraf(gitCloneDir, { disableGlob: true, maxBusyTries: 50, emfileWait: 2000 })
    function execute() {
      exec(`rmdir /S /Q my-test1`, function (error, stdout, stderr) {
        if (error) {
          console.error(error);
        }
        else {
